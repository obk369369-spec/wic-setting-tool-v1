...
const candidates=f.candidates.map((x,i)=>({...x,paid:true,tradable:true,publication_date:'2026',link:`https://example.com/${i}`,source_ref:`fixture:${i}`,provenance_verified:true,original_publisher_verified:true,official_detail_page:true,reseller_checked:true,is_reseller:false,toc_source_verified:true}));
...
for(const [field,value] of [['provenance_verified',false],['original_publisher_verified',false],['official_detail_page',false],['reseller_checked',false],['is_reseller',true],['toc_source_verified',false]]){
  const unsafe=candidates.map(x=>({...x,[field]:value}));
  const blocked=branchCustomer(f.base_state,unsafe);
  assert.equal(blocked.decision,'HOLD',field);assert.equal(blocked.next_action,'VERIFY_RECOMMENDATIONS',field);assert.equal(blocked.recommendations.length,0,field);
}
console.log("PASS: unverified publisher/detail/reseller/provenance/TOC candidates blocked 6/6");
